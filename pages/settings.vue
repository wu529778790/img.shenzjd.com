    ...
    >
      ...
      <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          >
            <div
              class="w-full h-12 bg-gradient-to-r from-gray-100 to-gray-800 rounded mb-2"
            ></div>
            <span class="text-sm font-medium text-gray-700 dark:text-gray-300"
              >自动</span
            >
  ...
  try {
    ...
    if (configStore.config) {
      if (configStore.config.image.watermark.text) {
        ...
        settings.value.watermarkEnabled =
          configStore.config.image.watermark.enabled;
      }
...
const saveSettings = async () => {
  ...
  try {
    ...
    if (configStore.config) {
      ...
      configStore.config.image.compressionQuality =
        settings.value.defaultCompression === "light"
          ? 0.85
          : settings.value.defaultCompression === "medium"
            ? 0.7
            : settings.value.defaultCompression === "heavy"
              ? 0.5
              : 0.92;
